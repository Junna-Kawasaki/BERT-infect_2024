...
MODEL_CLASSES = {
    ...
    "dnavibe":(ViBEConfig,ViBEForSequenceClassification,ViBETokenizer)
}
...
def train(args, train_dataset, model, tokenizer, class_wts):
    """ Train the model """
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            tmp_tr_loss, logits = outputs[:2]
            # binary cross entropy loss
            weights = torch.tensor([class_wts[int(label)] for label in inputs["labels"]], device=args.device)
            
            if args.n_gpu > 1:
                criterion = torch.nn.BCELoss(weight=weights, reduction="none")
            else:
                criterion = torch.nn.BCELoss(weight=weights, reduction="mean")
                
            probabilities = torch.sigmoid(logits[:, 1]).float()
            loss = criterion(probabilities, inputs["labels"].float())
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        results = evaluate(args, model, tokenizer, class_wts)
                        ...
                        if args.early_stop != 0:
                            ...
                            if results["auc"] < last_auc and results["f1"] < last_f1:
                                stop_count += 1
                            else:
                                stop_count = 0
...
def evaluate(args, model, tokenizer, class_wts, prefix="", evaluate=True):  ##class_wts, 
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                probabilities = torch.sigmoid(logits[:, 1]).float()
                BCE_loss = criterion(probabilities, inputs["labels"].float())

                #eval_loss += tmp_eval_loss.mean().item()
                eval_loss += BCE_loss.mean().item()
...
def main():
    ...
    if args.do_train and not args.do_cache:
        train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, evaluate=False)
        #print (train_dataset[0])
        class_l = [train_dataset[i][3].tolist() for i,_ in enumerate(train_dataset)]
        #print (class_l)
        class_wts = compute_class_weight(class_weight='balanced', classes=np.unique(class_l), y=class_l)
        class_wts = torch.tensor(class_wts, dtype=torch.float32, device=device)
        print (class_wts)
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, class_wts)
    ...
    if args.do_eval and args.local_rank in [-1, 0] and not args.do_cache:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, class_wts, prefix=prefix)
